...
in {
  ...
  config = mkIf (cfg.enable) (let xStartCommand = "${pkgs.dwmJD}/bin/dwm";
  in {
    assertions = [{
    ...
    home.packages = mkIf (cfg.type == "dwm")
      (with pkgs; [ dwmJD stJD dmenu xbindkeys xwallpaper ]);
    ...
    home.file = {
      ".xinitrc" = {
        ...
        text = ''
          ...
          ${if config.machineData.name == "framework" then
            "xrandr --output eDP-1 --scale 1.5x1.5"
          else
            ""}
          xset s ${toString cfg.screenlock.timeout.time} ${
            toString cfg.screenlock.lock.time
          }
    ...
    home.file.".xbindkeysrc" =
      mkIf (cfg.type == "dwm" && systemCfg.connectivity.sound.enable) {
        text = ''
          # Mute volume
    ...
    systemd = mkIf (cfg.type == "dwm" && cfg.screenlock.enable) {
      ...
      in {
        ...
        Service = {
          ExecStart =
            "${pkgs.picom}/bin/picom --config ${configFile} --experimental-backends";
          Restart = "always";
          RestartSec = 3;
