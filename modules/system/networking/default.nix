{ pkgs, config, lib, ... }:
with lib;
let cfg = config.thongpv87.networking;
in {
  options.thongpv87.networking = {
    interfaces = mkOption {
    ...
    static = {
      ...
      ipv4 = {
        ...
        onlink = mkOption {
          description =
            "Whether the gateway is not on the same network as the address";
          type = types.bool;
          default = false;
        };
  ...
  # {
  #   thongpv87.networking.allInterfaces = interfaces;
  #   networking = {
  ...
  #     matchConfig = { Name = cfg.static.interface; };
  ...
  # (mkIf (!config.thongpv87.unbound.enable) {
