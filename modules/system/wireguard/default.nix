...
with lib;
let
  wireguardConf = config.thongpv87.wireguard;

  inList = val: list: builtins.any (item: val == item) list;

  # Get the list of tag names from a conf
  getTagNames = peerConf: map (tag: tag.name) peerConf.tags;
  ...
  findFirstInSecond = firstList: secondList:
    findFirst (vFirst: builtins.any (vSecond: vFirst == vSecond) secondList)
  ...
  getTagConf = hostTags: peerConf:
    let
      ...
      tagName = findFirstInSecond hostTags (getTagNames peerConf);

      tagConf = findFirst (conf: conf.name == tagName)
        (builtins.throw "Missing tag in peer") peerConf.tags;
    in tagConf;

  # Filter out any peer config that does not intersect
  # tags with the given list of tags
  filterTags = hostTags: peerConfs:
    filterAttrs (_: peerConf:
      (builtins.any (peerTag: inList peerTag.name hostTags) peerConf.tags))
    peerConfs;

  # Build the peer configs
  buildPeers = with lib;
    myConf: peerConfs:
    let tags = getTagNames myConf;
    in mapAttrsToList (_: peerConf:
      (let
      ...
      in {
        wireguardPeerConfig = {
          ...
          Endpoint =
            "${tagConf.ipAddr}:${builtins.toString peerConf.listenPort}";
        };
  ...
  buildConfig = name:
    ...
    in {
      ...
      thongpv87.wireguard.allAddresses =
        mapAttrsToList (_: v: v.wgAddrV4) wireguardConf.peers;

      networking = {
        firewall.interfaces = let
          # allow wireguard listen port on each system interface
          openWireguard = listToAttrs (builtins.map (name: {
            inherit name;
            value = { allowedUDPPorts = [ myConf.listenPort ]; };
          }) config.thongpv87.networking.allInterfaces);
        in openWireguard // {
        ...
        networks."99-${wireguardConf.interface}" = {
          ...
          routes = [{
            routeConfig = {
              ...
              Scope = "link";
              Destination =
                "${myConf.wgAddrV4}/${builtins.toString myConf.interfaceMask}";
            };
          }];
          networkConfig = {
            Address = [ "${myConf.wgAddrV4}/32" ];
          } // optionalAttrs (myConf.dns == "client") {
            DNS = builtins.filter (v: !(builtins.isNull v)) (mapAttrsToList
              (_: peerConf:
                if (peerConf.dns == "server") then peerConf.wgAddrV4 else null)
              peerConfs);
            Domains = "~.";
            DNSSEC = true;
            DNSSECNegativeTrustAnchors =
              mapAttrsToList (_: peerConf: peerConf.domainName) peerConfs;
          };
        };
