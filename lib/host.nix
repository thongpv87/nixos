...
with builtins;
with utils; {
  mkISO = { name, initrdMods, kernelMods, kernelParams, kernelPackage
    , systemConfig, }:
    lib.nixosSystem {
      inherit system;
  ...
  mkHost = { name, initrdMods, kernelMods, kernelParams, kernelPackage
    , kernelPatches, systemConfig, cpuCores, stateVersion, wifi ? [ ]
    , passthru ? { }, gpuTempSensor ? null, cpuTempSensor ? null, }:
    let
      enable = [ "enable" ];
      ...
      systemConfigStripped = removeModuleOptions {
    ...
    in lib.nixosSystem {
      ...
      modules = [
        ...
        ++ [ inputs.impermanence.nixosModule ] ++ (systemEnableModule
